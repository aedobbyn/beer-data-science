...
cluster_prep <- cluster_prep(df = beer_totals,
                   preds = cluster_on,
                   to_scale = to_scale,
                   resp = response_vars)
...
cluster_it <- function(df_preds, n_centers) {
  ...
  clustered_df <- as_tibble(data.frame(
    ...
    cluster_prep$outcome, cluster_prep$preds,
    cluster_prep$df_for_clustering %>% select(abv, ibu, srm)))

  return(clustered_df)
}

clustered_beer <- cluster_it(df_preds = cluster_prep$preds, n_centers = 10)

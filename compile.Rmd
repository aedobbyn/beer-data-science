...
nn_collapsed_out$most_important_vars %>% get_nn_importance() %>% print()
```
...
nn_notcollapsed_out$most_important_vars %>% get_nn_importance() %>% print()
```
So style is harder to predict than collapsed style, which makes sense. 
...
Accuracy compared to that of the full random forest model, `r (1 - bi_rf$prediction.error)*100`:
```{r}
...
```{r, echo=TRUE}
get_importance(bi_pared_rf) %>% kable()
```

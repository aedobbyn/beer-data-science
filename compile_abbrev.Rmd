...
pick_ingredient_get_beer <- function (ingredient_want, df, grouper) {
  ...
  get_ingredients_per_grouper <- function(df, grouper = grouper) {
    ...
    per_grouper <- df_grouped %>% 
      # select_if(!grepl("hops|malt|abv|ibu|srm|Id|id", names(.))) %>%
    ...
    not_for_summing <- which(colnames(per_grouper) %in% to_keep_col_names)
    max_not_for_summing <- max(not_for_summing)
    
    per_grouper_w_totals <- per_grouper %>%
      mutate(
    ...
    per_grouper_reorder <- per_grouper_w_totals %>% 
      select(
        name, total, everything()
      )
    
    # Replace total column with more descriptive name: total_<ingredient>
    names(per_grouper_reorder)[which(names(per_grouper_reorder) == "total")] <- paste0("total_", ingredient_want)
    
    return(per_grouper_reorder)
  }
  # ------------------------------------------------------------------------------- # 
  
  ingredients_per_grouper <- get_ingredients_per_grouper(beer_spread_no_na, grouper)
